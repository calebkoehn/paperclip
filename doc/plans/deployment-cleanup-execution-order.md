...
### Render incident follow-up (2026-03-12)

Observed failure after UI build success and during server TypeScript compile:

- `TS2307: Cannot find module '@paperclipai/adapter-gemini-local/server'`
- `TS2307: Cannot find module '@paperclipai/adapter-gemini-local'`

Root cause: `server/src` imports Gemini adapter modules while `server/package.json` did not declare `@paperclipai/adapter-gemini-local`.

Mitigation implemented:

1. Restore Gemini dependency in `server/package.json`.
2. Restore Gemini dependency in `cli/package.json` (CLI registry imports Gemini adapter formatter).
3. Extend `check:workspace-deploy` to detect adapter imports in source that are missing from package dependencies.
4. Add explicit `pnpm --filter @paperclipai/server build` CI step in PR/release verification for earlier signal.
